...
local plugin_name       = "openid-connect-multi-realm"
...
-- Create a route-specific realm configurations cache
local route_realm_configs = {}

-- Initialize realm configurations dictionary for a specific route
local function initialize_realm_configs(realms, route_id)
  local realm_configs = {}
  
  for _, config in ipairs(realms) do
      realm_configs[config.realm] = config
  end
  
  -- Store realm configs for this specific route
  route_realm_configs[route_id] = realm_configs
  core.log.info("Initialized " .. #realms .. " realm configurations for route " .. route_id)
end

function get_realm_config(realm, route_id)
  local realm_configs = route_realm_configs[route_id]
  if not realm_configs or not realm_configs[realm] then
    return
  end
...
function _M.check_schema(conf)
    ...
    -- Note: check_https and check_tls_bool functions not available in this APISIX version
...
function _M.rewrite(plugin_conf, ctx)
    -- Get route ID for route-specific realm configuration
    local route_id = ctx.route_id
    if not route_id then
        core.log.error("Route ID not found")
        return ngx.HTTP_UNAUTHORIZED
    end
    -- Initialize realm configurations for this specific route
    initialize_realm_configs(plugin_conf.realms, route_id)
    local realm_name = core.request.header(ctx, "X-Realm")
    if not realm_name then
        core.log.error("Realm header not found")
        return ngx.HTTP_UNAUTHORIZED
    end
    
    local realm_config = get_realm_config(realm_name, route_id)
    if not realm_config then
        return ngx.HTTP_UNAUTHORIZED
    end
    local realm_conf_clone = core.table.clone(realm_config)
    local conf = fetch_secrets(realm_conf_clone, true, realm_config, "")
